...
class BinanceSptHttpClient(HttpWrapper, ForceAsyncNew):
    ...
    async def New(
        ...
        **kwargs,
        ...
        """Http client for Binancer REST API."""
        ...
        instance._api_limit_weight_max = 6000
        instance._api_limit_weight = 6000
        instance._api_limit_cond = asyncio.Condition()

        async def _refresh_api_limit():
            weight_refill_per_period = 10  # 6000 weight per 1 minute = 6000 weight per 60 seconds = 100 weight per second
            try:
                while True:
                    await asyncio.sleep(
                        0.1
    ...
    async def request(  # type: ignore
        ...
        """Make an authenticated request to the Binancer API, respecting the API rate limit.
    ...
    async def request_unsafe(
        ...
        """unsafe, Make an authenticated request to the Binancer API.
...
class BinanceWsClient(WebsocketWrapper):
    """Websocket client for Binancer websocket API.

    use .New() to create an instance of this class.
    """

    _callbacks: dict[str, Callable[[Self, Dict], None]]
    """Dictionary mapping channel names to callback functions. The callback function is called when a message with the corresponding channel is received. Signature of callback functions: (BinanceWsClient, Dict) -> None"""
    _callback_default: Callable[[Self, Dict], None]
    """Default callback function that is called when a message with an unregistered channel is received. Signature: (BinanceWsClient, Dict) -> None"""
    ...
    async def New(
        ...
        url: str = "wss://api.Binancer.xyz/ws",
    ) -> Self:
        """Websicjet client for Binancer websocket API.

        Args:
            on_connected: Callback function that is called when the websocket connection is established. Signature: (BinanceWsClient) -> None
            on_disconnected: Callback function that is called when the websocket connection is disconnected. Signature: (BinanceWsClient) -> None
            on_frame: Dictionary mapping channel names to callback functions. The callback function is called when a message with the corresponding channel is received. Signature of callback functions: (BinanceWsClient, Dict) -> None
            on_frame_default: Default callback function that is called when a message with an unregistered channel is received. Signature: (BinanceWsClient, Dict) -> None
            url: Websocket URL to connect to. Default is "wss://api.Binancer.xyz/ws".
        """
    ...
    def subscribe(self, message: str):
        ...
        https://Binancer.gitbook.io/Binancer-docs/for-developers/api/websocket/subscriptions
        """
